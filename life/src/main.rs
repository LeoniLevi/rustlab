...
fn main() {
    ...
    let world1 = generation(world);
    println!(" ~~ func. generation is called!");
}
*/
...
fn displayworld(world: [[u8; 75]; 75])
{
    for i in 0..74 {
        for j in 0..74 {
            if world[i][j] == 1 {
                //print!("{red}*", red = termion::color::Fg(termion::color::Red));
                print!("{yellow}*", yellow = termion::color::Fg(termion::color::LightYellow));
            } else {
                print!(" ");
            }
        }   
...
fn main() {
    ...
    for _gens in 0..100 {
        ...
        displayworld(world);
        println!("{white}Population at generation {g} is {c}", 
